...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			TRIM(cst_lastname) AS cst_lastname, --Removes unwanted spaces
		...
		WHERE flag_last = 1; --Eliminates Duplicates by selecting most recent record per customer
		...
		END AS sls_order_dt, -- Nulling improper dates and casting others as Dates
		CASE WHEN sls_ship_dt = 0 OR LEN(sls_ship_dt) != 8 THEN NULL
			ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
		END AS sls_ship_dt, -- Nulling improper dates and casting others as Dates
		CASE WHEN sls_due_dt = 0 OR LEN(sls_due_dt) != 8 THEN NULL
			ELSE CAST(CAST(sls_due_dt AS VARCHAR) AS DATE)
		END AS sls_due_dt, -- Nulling improper dates and casting others as Dates
		CASE WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_quantity * ABS(sls_price)
				THEN sls_quantity * ABS(sls_price)
			ELSE sls_sales
		END AS sls_sales, -- Calculate sales if original value is missing or incorrect
		sls_quantity,
